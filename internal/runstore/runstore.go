...
// Returns an error if id fails the same path-traversal validation as safeRunDir.
func (s *Store) WorkDirForRun(id string) (string, error) {
	runDir, err := s.safeRunDir(id)
	if err != nil {
		return "", err
	}
	return filepath.Join(runDir, "workdir"), nil
}
